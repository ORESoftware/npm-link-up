#!/bin/bash
#
# Bash completion generated for 'nlu' at Thu Jul 12 2018 14:34:44 GMT-0700 (Pacific Daylight Time).
#
...
#   1. Uncomment the "_nlu_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_nlu_log=/var/tmp/dashdash-completion.log

function _nlu_completer {

    # ---- cmd definition

    local cmd_shortopts="-h -h -h -h -v -v -v -v"
local cmd_longopts="--append-search --clear-all-caches --clear-caches --dry --dry --dry-run --dry-run --force --force --force --force --help --help --help --help --install --install-all --install-main --install:all --install:main --interactive --link --link-all --link-main --link:all --link:main --no-local --no-local --no-use-local --no-use-local --npm-shell-version --npm-shell-version --override --prod --production --search --search --search-append --search-from-home --search-root --search-root --search-root-append --shell-version --shell-version --treeify --use-shell-version --use-shell-version --verbosity --verbosity --verbosity --verbosity --version --vn --yarn"
local cmd_optargs="--append-search=arrayOfString --search-append=arrayOfString --search-root-append=arrayOfString --search-root=arrayOfString --search-root=arrayOfString --search=arrayOfString --search=arrayOfString --verbosity=positiveInteger --verbosity=positiveInteger --verbosity=positiveInteger --verbosity=positiveInteger -v=positiveInteger -v=positiveInteger -v=positiveInteger -v=positiveInteger"
...
if type complete &>/dev/null; then
    function _nlu_completion {
        local _log_file=/dev/null
        [[ -z "$_nlu_log" ]] || _log_file="$_nlu_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _nlu_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _nlu_completion nlu
    ...
    function _nlu_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _nlu_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _nlu_completion nlu
    ...
    compctl -K _nlu_completion nlu
fi


##
## This is a Bash completion file for the 'nlu' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/nlu   # Mac
##     cp FILE /etc/bash_completion.d/nlu             # Linux
##
## or:
##
##     cp FILE > ~/.nlu.completion
##     echo "source ~/.nlu.completion" >> ~/.bashrc
##
