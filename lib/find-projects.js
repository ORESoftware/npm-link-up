...
exports.makeFindProject = function (totalList, map, ignore, opts) {
    ...
    return function findProject(item, cb) {
        ...
        if (searchedPaths[item]) {
            logging_1.log.good('already searched this path, not searching again:', chalk.bold(item));
        ...
        if (match) {
            logging_1.log.good('path has already been covered:');
            logging_1.log.good('new path:', item);
            logging_1.log.good('already searched path:', goodPth);
            return process.nextTick(cb);
        }
