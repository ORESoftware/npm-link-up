...
#   1. Uncomment the "_npmlinkup_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_npmlinkup_log=/var/tmp/dashdash-completion.log

function _npmlinkup_completer {
...
if type complete &>/dev/null; then
    function _npmlinkup_completion {
        local _log_file=/dev/null
        [[ -z "$_npmlinkup_log" ]] || _log_file="$_npmlinkup_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _npmlinkup_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _npmlinkup_completion npmlinkup
    ...
    function _npmlinkup_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _npmlinkup_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _npmlinkup_completion npmlinkup
    ...
    compctl -K _npmlinkup_completion npmlinkup
fi


##
## This is a Bash completion file for the 'npmlinkup' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/npmlinkup   # Mac
##     cp FILE /etc/bash_completion.d/npmlinkup             # Linux
##
## or:
##
##     cp FILE > ~/.npmlinkup.completion
